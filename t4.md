...
PHP permite configurar varios aspectos de la sesión mediante la función `ini_set()` o en el archivo de configuración `php.ini`.

```php
<?php
// Cambiar la duración de la cookie de sesión
...
### 6. **ID de sesión**
Cada sesión tiene un identificador único que se puede obtener o regenerar.
...
Es una buena práctica para mejorar la seguridad al cambiar el ID de sesión regularmente, especialmente después de un inicio de sesión exitoso.
...
### 7. **Sesiones seguras (mejoras en PHP 8)**
PHP 8 ha mejorado la seguridad en el manejo de sesiones, como la **propagación segura del ID de sesión**. Para mayor seguridad:
- **Usar cookies seguras** (`session.cookie_secure = 1`).
- **Usar HTTP only** (`session.cookie_httponly = 1`).
- **Deshabilitar la propagación del ID de sesión en la URL** (`session.use_only_cookies = 1`).
